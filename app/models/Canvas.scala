...
class Canvas private (
    ...
    _activeUsers: Option[List[UserInfo]] = Some(List[UserInfo]()), 
  ...
  private var activeUsers = if(_activeUsers.isDefined) _activeUsers.get else List[UserInfo]()
  ...
  def addMember(user: UserInfo) = {
    activeUsers ::= user
  }
  
  def removeMember(userId: Int) = {
    activeUsers = activeUsers.filterNot(u => u.id == userId)
  }
  ...
  def getJsValue: JsValue = {
    ...
    JsObject(Seq(
      ...
      "activeUsers" -> JsObject(userJson.toSeq),
...
object Canvas {
  ...
  def getCanvasOptFromJson(json: JsValue): Option[Canvas] = {
    ...
    val activeUsersOpt = (json \ "activeUsers").asOpt[JsObject]
    ...
    if(canvasIdOpt.isEmpty) {
      ...
      val activeUsers = if(activeUsersOpt.isDefined) Some(activeUsersOpt.get.value.map(pair => new UserInfo(pair._1.toInt,pair._2.as[JsString].value)).toList) else None
      ...
      Some(new Canvas(canvasId, timeCreatedMillis, activeUsers, particleActions, tags))
    }
  }
}